...
with tab5:
    ...
    st.caption("⚠️ Research only — not financial advice. Verify all data independently.")import pandas as pd
import numpy as np
import time
...
def av_get(function, ticker, api_key, **kwargs):
    """
    Single AV endpoint call with error handling.
    ttl=3600 (1 hour) — avoid re-fetching the same ticker within a session.
    Free tier: 25 calls/day, 5/min. Calls are counted in the sidebar meter.
    """
    params = {"function": function, "symbol": ticker, "apikey": api_key}
    params.update(kwargs)
    r = requests.get(BASE, params=params, timeout=15)
    ...
    if "Note" in data:
        raise RuntimeError(
            "⏳ Alpha Vantage rate limit hit (5/min). Wait 60 seconds and try again."
        )
    if "Information" in data:
        raise RuntimeError(
            "🔑 Alpha Vantage daily limit reached (25 calls/day on free tier). "
            "Try again tomorrow or upgrade at alphavantage.co/premium/"
        )
    if "Error Message" in data:
        raise RuntimeError(f"❌ Ticker '{ticker}' not found. Check the symbol.")
...
def fetch_data(ticker, api_key, mode):
    """
    Tiered fetch: only call the endpoints the selected mode requires.
    A 0.5-second pause between calls respects the 5/min burst limit safely.

    Returns a dict of raw AV responses keyed by endpoint name.
    Missing endpoints return empty dicts — all downstream code handles gracefully.
    """
    raw = {
        "overview": {}, "income": {}, "balance": {},
        "cashflow": {}, "quote": {}, "daily": {}
    ...
    def get_with_pause(fn, key, **kw):
        raw[key] = av_get(fn, ticker, api_key, **kw)
        time.sleep(0.5)   # stay well under 5 calls/min burst limit

    # Mode 1 — always fetch (2 calls)
    get_with_pause("OVERVIEW",         "overview")
    get_with_pause("INCOME_STATEMENT", "income")

    # Mode 2 — balance sheet + cash flow (2 more calls)
    if mode in ("Full (4 calls)", "Chart (6 calls)"):
        get_with_pause("BALANCE_SHEET", "balance")
        get_with_pause("CASH_FLOW",     "cashflow")

    # Mode 3 — live quote + price chart (2 more calls)
    if mode == "Chart (6 calls)":
        get_with_pause("GLOBAL_QUOTE",      "quote")
        get_with_pause("TIME_SERIES_DAILY", "daily", outputsize="compact")
...
with st.sidebar:
    ...
    # Call budget meter
    calls_this = {"Lean (2 calls)": 2, "Full (4 calls)": 4, "Chart (6 calls)": 6}[mode]
    remaining  = st.session_state.get("calls_used_today", 0)
    budget_left = max(25 - remaining, 0)
    analyses_left = budget_left // calls_this

    st.markdown(
        f'<div class="call-meter">'
        f'🔋 Budget: <b>{budget_left}/25</b> calls left today<br>'
        f'📊 This run: <b>{calls_this} calls</b><br>'
        f'🔄 Analyses left: <b>~{analyses_left}</b> at this mode'
        f'</div>',
        unsafe_allow_html=True
    )
    if analyses_left <= 2:
        st.warning("⚠️ Running low on daily calls. Use Lean mode to conserve.")
...
if run:
    st.session_state["last_ticker"] = ticker_input
    st.session_state["last_mode"]   = mode
    # Track call usage (resets when the app restarts / new session)
    st.session_state["calls_used_today"] = \
        st.session_state.get("calls_used_today", 0) + calls_this
    st.cache_data.clear()
